...
const upgradeRedUser = `-- name: UpgradeRedUser :exec
UPDATE users
  SET is_chirpy_red = true
WHERE id = $1
`

func (q *Queries) UpgradeRedUser(ctx context.Context, id uuid.UUID) error {
	_, err := q.db.ExecContext(ctx, upgradeRedUser, id)
	return err
}
